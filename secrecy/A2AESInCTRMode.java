        ...
        env.add(new Agent("alice") {
            ...
            public void task() throws Exception {
                ...
                for (int i=0;i<num_rounds;i++) {
                    ...
                    // send the IV
                    final byte[] iv_A = new byte[16];
                    SecureRandom.getInstanceStrong().nextBytes(iv_A);
                    send("bob", iv_A);


                    encrypt.init(Cipher.ENCRYPT_MODE, key, new IvParameterSpec(iv_A));
        ...
        env.add(new Agent("bob") {
            ...
            public void task() throws Exception {
                ...
                for (int i=0;i<num_rounds;i++) {
                    ...
                    // send the IV
                    final byte[] iv_B = new byte[16];
                    SecureRandom.getInstanceStrong().nextBytes(iv_B);
                    send("alice", iv_B);

                    encrypt.init(Cipher.ENCRYPT_MODE, key, new IvParameterSpec(iv_B));
                    final byte[] pt = message.getBytes();
